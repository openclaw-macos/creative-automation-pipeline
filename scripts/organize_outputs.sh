#!/bin/bash
# Organize outputs for Creative Automation Pipeline
...
for file in "$PROJECT_ROOT"/*.txt "$PROJECT_ROOT"/*.log "$PROJECT_ROOT"/outputs/*.txt; do
...
if [ -d "$PROJECT_ROOT/outputs/logs" ]; then
    for file in "$PROJECT_ROOT/outputs/logs"/*.json "$PROJECT_ROOT/outputs/logs"/*.db; do
        if [ -f "$file" ] 2>/dev/null; then
...
# echo " optional section in scripts/organize_outputs.sh (lines 170-185)."
# echo ""
...
echo ""
echo "🎯 Tip: Add './scripts/organize_outputs.sh' to your .gitignore if you want to keep"
